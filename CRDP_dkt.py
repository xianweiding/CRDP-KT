...
# divide train test set 划分训练，测试的设置
...
# embed data, used for initialize 嵌入数据用于初始化
...
with tf.compat.v1.variable_scope('pro_skill_embed', reuse=tf.compat.v1.AUTO_REUSE):
    if use_pretrain:
    ...
    pro_embedding_matrix = tf.compat.v1.get_variable('pro_embed_matrix', [pro_num, embed_dim],
    ...
    # deal with complement symbol 处理补码符号
...
# concat zero on the skill embedding 在技能嵌入上连接零
def concat_zero(x):
    xx = x[:-1]
    yy = x[-1]
...
# lstm
with tf.compat.v1.variable_scope('LstmNet', reuse=tf.compat.v1.AUTO_REUSE):
    lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_dim, state_is_tuple=True)
    init_state = lstm_cell.zero_state(tf_batch_size, tf.float32)
    outputs, last_state = tf.compat.v1.nn.dynamic_rnn(cell=lstm_cell,
                                            inputs=rnn_inputs,
    ...
    rnn_w = tf.compat.v1.get_variable('softmax_w', [hidden_dim+inputs_embed_dim, 1], initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
    rnn_b = tf.compat.v1.get_variable('softmax_b', [1], initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
    logits = tf.compat.v1.nn.xw_plus_b(outputs_reshape, rnn_w, rnn_b)  # [bs*(max_len-1), 1]
...
filtered_logits = tf.squeeze(tf.gather(logits, index), axis=1)#t.gather()为切片操作
final_logits = filtered_logits

# cross entropy, optimize 交叉熵，优化
...
if train_flag:
    # begin train 开始训练
    train_steps = int(math.ceil(train_skill.shape[0]/float(bs)))#match.ceil()向上取整
    ...
    with tf.compat.v1.Session() as sess:
        ...
        best_auc = best_acc = 0
        for i in tqdm(range(epochs)):#tqdm()函数用于显示进度条
            train_loss = 0
